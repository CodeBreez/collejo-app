...
class EmployeeController extends BaseController
{
	...
	public function getEmployeeAddressEdit($employeeId, $addressId)
	{
		$this->authorize('edit_employee_contact_details');

        return $this->printModal(view('employees::modals.edit_address', [
                'address' => $this->employeeRepository->findAddress($addressId, $employeeId),
                'employee' => $this->employeeRepository->findemployee($employeeId)
            ]));
	}

	public function postEmployeeAddressEdit(UpdateAddressRequest $request, $employeeId, $addressId)
	{
		$this->authorize('edit_employee_contact_details');

        $address = $this->employeeRepository->updateAddress($request->all(), $addressId, $employeeId);

        return $this->printPartial(view('employees::partials.address', [
                'employee' => $this->employeeRepository->findemployee($employeeId),
                'address' => $address
            ]), trans('employees::address.address_updated'));
	}

	public function getEmployeeAddressDelete($employeeId, $addressId)
    {
    	$this->authorize('edit_employee_contact_details');

        $this->employeeRepository->deleteAddress($addressId, $employeeId);

        return $this->printJson(true, [], trans('employees::address.address_deleted'));
	}
